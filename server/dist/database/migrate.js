...
const log = (0, logger_1.createLogger)('migration');
const SCHEMA_SQL = `
-- Users
...
function runMigrations() {
    ...
    const existingFlags = db.prepare('SELECT COUNT(*) as count FROM feature_flags').get();
    if (existingFlags.count === 0) {
        db.prepare(`
      INSERT INTO feature_flags (id, feature_key, display_name, description, is_enabled, is_visible)
      VALUES (?, ?, ?, ?, 0, 0)
    `).run('ff-bluetooth', 'BLUETOOTH_ENABLED', 'Bluetooth Client-Server Transport', 'Fully implemented Bluetooth transport between client and server. Disabled and hidden by default — only Super Administrator can enable.');
